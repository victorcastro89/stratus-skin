...
| `instructions/skin-styles.instructions.md` | **You open/edit a file in `roundcubemail/skins/stratus/styles/`** | Copilot auto-loads LESS coding rules (use `@` not `$`, `mp-` prefix, dark mode patterns). Triggered by the `applyTo` glob in the file's frontmatter. |
| `instructions/skin-templates.instructions.md` | **You open/edit a file in `roundcubemail/skins/stratus/templates/`** | Copilot auto-loads Roundcube template tag rules. |
| `instructions/plugin-php.instructions.md` | **You open/edit a file in `roundcubemail/plugins/stratus_helper/`** | Copilot auto-loads PHP plugin API rules. |
...
```
You:  @builder start Phase 1
      → reads roadmap → creates meta.json, LESS files, templates → compiles → validates → updates memory
...
You:  @builder continue
      → reads memory → picks up next uncompleted items → builds → compiles → done
```
...
You:  @builder override the login page with a centered card layout and add styles for it
      → reads elastic's login.html → creates template override → writes _login.less → compiles → done
```

### Flow 4: Focused Style Exploration
...
| Prompt | What it does |
|--------|-------------|
| **`/build-next`** | **Primary — pick up next roadmap items, build, compile, validate, update memory** |
...
grep "mp-" roundcubemail/skins/stratus/styles/styles.min.css

# Check CSS file size
wc -c roundcubemail/skins/stratus/styles/styles.min.css
```
