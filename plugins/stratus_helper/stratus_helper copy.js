...
(function () {
  ...
  function initSearchEmptyState() {
    ...
    function isConvMode() {
      var ll = document.getElementById('layout-list');
      return ll && ll.getAttribute('data-conv-mode') === 'conversations';
    }
    ...
    rcmail.addEventListener('listupdate', function (evt) {
      if (!container || isConvMode()) return;
  ...
  function initUnifiedHoverActions() {
    ...
    function isConvRow(row) {
      return !!(row && row.getAttribute('data-conv-id'));
    }
    ...
    function createHoverActions(row) {
      ...
      archiveBtn.addEventListener('click', function (e) {
        ...
        dbg('row.id=', row.id, 'conv?', isConvRow(row));
        groupEnd();

        if (isConvRow(row)) {
          var convId = row.getAttribute('data-conv-id');
          if (rcmail.conv_api) rcmail.conv_api.select_conv(convId);
      ...
      deleteBtn.addEventListener('click', function (e) {
        ...
        if (isConvRow(row)) {
          var convId = row.getAttribute('data-conv-id');
          if (rcmail.conv_api) rcmail.conv_api.select_conv(convId);
          rcmail.command('plugin.conv.delete');
          return;
        }
      ...
      flagBtn.addEventListener('click', function (e) {
        ...
        if (isConvRow(row)) {
          var convId = row.getAttribute('data-conv-id');
          if (rcmail.conv_api) rcmail.conv_api.select_conv(convId);
          rcmail.command('plugin.conv.flag');
          return;
        }
    ...
    if (convList) processRows(convList);
  ...
  function initSmartBarController() {
    ...
    if (pluginSlots && smartBar) {
      ...
      if (convToggle) convToggle.style.display = 'none';
      ...
      if (defaultSection) {
        ...
        for (var i = 0; i < remainingBtns.length; i++) {
          ...
          if (btn.classList.contains('conv-toggle')) continue;
